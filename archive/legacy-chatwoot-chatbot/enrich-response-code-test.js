...
// FR-6.1: Out-of-hours detection for booking confirmation
...
// FR-3.2: Residual BOOKING_CONFIRM — malformed or missing JSON gets generic confirmation
...
// FR-3.1: Catch-all — strip any unrecognized markers so raw text never reaches user
...
// FR-3.3: If enriched became empty after stripping (e.g. response was only an unrecognized marker)
if (!enriched.trim() && !quickHtml) {
  return 'Atsiprašome, šiuo metu negaliu atsakyti.';
}
