...
if (!response || !response.trim()) {
  return [{ json: { enriched: 'Atsiprašome, šiuo metu negaliu atsakyti. Susisiekite tiesiogiai: +370 648 803 88 arba info@batutynas.lt' } }];
}
...
function buildTrampolineCards(items, highlight, multiSelect, compact) {
  ...
  for (const t of items) {
    const thumb = t.img
      ? '<img src="' + escapeHtml(t.img) + '" alt="' + escapeHtml(t.name) + '" data-chat-zoom>'
      : escapeHtml(t.icon);
    const bgClass = ' t-bg-' + t.cat;
    const recClass = highlight ? ' t-recommended' : '';
    const selectAttr = multiSelect
      ? 'data-chat-addon="' + escapeHtml(t.name) + '"'
...
function buildGroupPartyEquipment() {
  ...
  html += buildTrampolineCards(party, false, false, false);
  return html;
}
...
function buildDatePicker() {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    days.push('<button class="chat-option-btn" data-chat-option="' + escapeHtml(iso) + '">' + escapeHtml(label) + '</button>');
    d.setDate(d.getDate() + 7);
  }
  const nowIso = escapeHtml(localIso(now));
  let html = '<div class="chat-options">' + days.join('') + '</div>';
  html += '<input type="date" class="chat-date-input" data-chat-date min="' + nowIso + '" placeholder="Kita data...">';
...
function buildLocationOptions() {
  ...
  for (const loc of locs) {
    html += '<button class="chat-option-btn" data-chat-address-fill="' + escapeHtml(loc) + '">' + escapeHtml(loc) + '</button>';
  }
  html += '</div>';
...
function buildGuestCountOptions() {
  ...
  for (const r of ranges) {
    html += '<button class="chat-option-btn" data-chat-option="' + escapeHtml(r.value) + '">' + escapeHtml(r.label) + '</button>';
  }
  html += '</div>';
  return html;
}
...
function buildBookingConfirm(jsonStr) {
  ...
  if (isOutOfHours()) {
    html += '<p class="booking-confirm-hours">⏰ Užklausa bus apdorota artimiausią darbo dieną (I–V, 8:00–18:00)</p>';
  }
  html += '</div>';
  return html;
}
...
function buildMainMenu() {
  ...
  for (const item of items) {
    html += '<button class="chat-option-btn chat-menu-btn" data-chat-option="' + escapeHtml(item.value) + '">' + escapeHtml(item.label) + '</button>';
  }
  html += '</div></div>';
  return html;
}
...
function buildQuickReplies(buttons) {
  ...
  for (const btn of buttons) {
    ...
    html += '<button class="' + btnClass + '" data-chat-option="' + escapeHtml(value) + '">' + escapeHtml(label) + '</button>';
  }
  html += '</div>';
  return html;
}
...
// Note: LOCATION_OPTIONS marker removed — no longer present in prompts.
const markers = [
...
// Regex supports nested braces (e.g. values containing {}) via: \{[^}]*(?:\{[^}]*\}[^}]*)*\}
...
if (!enriched.trim() && !quickHtml) {
  enriched = 'Atsiprašome, šiuo metu negaliu atsakyti. Susisiekite tiesiogiai: +370 648 803 88 arba info@batutynas.lt';
